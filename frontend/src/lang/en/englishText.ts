// Our Mission Text
export const missionText = `
<p>Hli Haykwhl Ẃii Xsgaak Consulting is an Indigenous owned and matriarch-led corporation dedicated to advancing the 94 Calls to Action of the Truth and Reconciliation Commission. Our mission is to guide, empower, and inspire people and organizations to be allies and agents for change while making transformational community impact.</p>
<br/>
<p>Led by Melanie Mark, a dynamic changemaker with over 30 years of experience across nonprofit, public service, and private sectors. Known for her relentless advocacy and ability to turn complex ideas into actionable results, Melanie is a trusted ally who prioritizes rights, relationships, and results.</p>    
`

// About Text
export const aboutText = `<p>
                        I’m Melanie Mark, a proud mother of two daughters and decendent of
                        the Nisga’a, Gitxsan, Cree, and Ojibway First Nations. Born and
                        raised in East Vancouver, I’ve lived and worked throughout B.C. and
                        Canada, cultivating meaningful relationships in various sectors. My
                        late grandparents are from Laxalxap, Gitanmaax and Peguis
                        First Nations and attended St. Michael’s, Elkhorn and Brandon Indian
                        Residential Schools.
                    </p>
                    <br/>
                    <p>
                        Over the past three decades, I’ve served as an advocate, public
                        servant, elected official, and now entrepreneur in the cleantech
                        space. My journey has been guided by a commitment to justice,
                        empowerment, and systemic change.
                    </p>
                    <br/>
                    <p>
                        As the first First Nations woman elected to the B.C. Legislature and
                        appointed to Cabinet, I’ve had the privilege of working with diverse
                        communities, from grassroots organizations to executive leadership.
                        Now, as the founder of Hli Haykwhl Ẃii Xsgaak Consulting, I’m sharing
                        my knowledge and experiences to help others achieve their goals
                        through the power of culture, sport, education and industry.
                    </p>
                    <br/>
                    <p class='specialFont ql-align-right'>
                        <strong>CEO, Melanie Mark,</strong>
                    </p>
                    <p class='specialFont ql-align-right'>
                        <strong>Hli Haykwhl Ẃii <u>X</u>sgaak Consulting</strong>
                    </p>`

// Services Texts
export const serviceAdvisoryText = `<b class='block font-bold text-xl mb-2'>Governmental and Non-Partisan Political Relations</b>
<ul class='list-disc list-inside space-y-2 text-lg'>
    <li>I’ll offer high-level strategic advice to guide you through complex systems and successfully implement change.</li>
    <li>I’ll direct you to the right organization, decision makers, government body, business, public institution, or First Nation to advance your ideas.</li>
    <li>I’ll support you in narrowing your 30 big ideas into 3 attainable outcomes.</li>
    <li>I’ll review your draft documents and provide advice on how to prepare, approach, and engage with the B.C. public service, B.C. MLAs, Official Opposition, Cabinet Ministers, First Nations Leadership, and other stakeholders.</li>
    <li>I’ll keep your approach focused and efficient, ensuring your time is used wisely.</li>
</ul>
`;

export const serviceWorkshopText = `<b class='block font-bold text-xl mb-2'>Paddling Together: Team Building & Community Engagement</b>
<ul class='list-disc list-inside space-y-2 text-lg'>
    <li>I’ll facilitate in-person and virtual workshops to guide organizations committed to advancing the Truth and Reconciliation Commission’s 94 Calls to Action.</li>
    <li>I’ll aspire to educate others by sharing my experience as an inter-generational Indian Residential school survivor, unpacking what we were never taught.</li>
    <li>I’ll advise you on how to build your community network, strengthen relationships, and connect with the right people and organizations.</li>
</ul>

<b class='block font-bold text-xl mb-2 mt-6'>Leadership Empowerment & Public Speaking Coaching</b>
<ul class='list-disc list-inside space-y-2 text-lg'>
    <li>I’ll support your leadership journey and assist with developing public speaking skills, keynotes, and media engagement with confidence and time efficiency.</li>
    <li>I’ll offer one-on-one or small group coaching on how to achieve a work-life balance, take control of your time, and consistently show up as your best self.</li>
</ul>

<b class='block font-bold text-xl mb-2 mt-6'>Event Planning & Coordination</b>
<ul class='list-disc list-inside space-y-2 text-lg'>
    <li>I’ll guide you to successfully plan and execute the most effective location, date, time, menu, guest list, and program for your event.</li>
    <li>I’ll support you through logistics, accessibility considerations, speaker selection, and a personalized communications/marketing strategy to ensure intentionality and profound impact.</li>
</ul>`;

export const servicePublicSpeakText = `<b class='block font-bold text-xl mb-2'>Multidimensional Leadership</b>
<ul class='list-disc list-inside space-y-2 text-lg'>
    <li>I’ll deliver highly emotional, thought-provoking, and motivational keynotes guaranteed to make you laugh, cry, or cheer.</li>
    <li>I’ll moderate panels and emcee events that evoke lasting impressions.</li>
    <li>I’ll speak about almost any topic/theme related to reconciliation and confidently engage diverse audiences through remarkable storytelling and sharing tangible lessons learned. </li>
    <li>I’ll commit to sparking new conversations that inspire change, boost morale, and encourage actionable steps towards reconciliation.</li>
    <li>I’ll respect your time and your audience, ensuring I prioritize your unique event goals and individual budget.</li>
</ul>`;


// Experience Text
export const defaultExperienceText = `
<ul className="list-disc list-outside space-y-4 text-lg leading-relaxed">
    <li>
        A results-oriented, charismatic, determined, knowledgeable, and
        experienced First Nation’s matriarch with re<strong>MARK</strong>able leadership,
        strategic thinking, advocacy, public speaking, communication,
        facilitation, and interpersonal skills.
    </li>
    <li>
        A pragmatic visionary leader, with a strong acumen navigating
        complex systems and public relations, supported by a solid track
        record of advancing public policy, programs, and provincial
        legislation.
    </li>
    <li>
        An Elder-in-training, with a deep understanding of colonialism and
        the devastating impacts of the Indian residential schools; actively
        cultivating personal/Indigenous people’s history, cultural
        practices, protocols, customs, and traditions through family,
        community, post-secondary education, and professional teachings.
    </li>
    <li>
        Spent three decades building respectful and meaningful
        relationships with local xʷməθkʷəy̓əm (Musqueam), Sḵwx̱wú7mesh
        (Squamish), and səlilwətaɬ (Tsleil-Waututh) Nations and
        elected/hereditary First Nation leaders across B.C. based on the
        principles of Rights, Recognition, and Respect.
    </li>
</ul>
`;


// Resume Texts
export const resumeProfileText = `
  <h3 class="text-2xl font-light text-brandRed mb-6 text-center">Profile</h3>
  <ul class="list-disc list-outside space-y-4 text-lg leading-relaxed">
    <li>Committed to advancing meaningful social, environmental, and economic justice through reconciliation</li>
    <li>A results-oriented, charismatic, determined, knowledgeable, and experienced First Nation’s matriarch with re<strong>MARK</strong>able leadership, strategic thinking, advocacy, public speaking, communication, facilitation, and interpersonal skills</li>
    <li>A pragmatic visionary leader, with a strong acumen navigating complex systems and public relations, supported by a solid track record of advancing public policy, programs, and provincial legislation</li>
    <li>An Elder-in-training, with a deep understanding of colonialism and the devastating impacts of the Indian residential schools; actively cultivating personal/Indigenous people’s history, cultural practices, protocols, customs, and traditions through family, community, post-secondary education, and professional teachings</li>
    <li>Spent three decades building respectful and meaningful relationships with local xʷməθkʷəy̓əm (Musqueam), Sḵwx̱wú7mesh (Squamish), and səlilwətaɬ (Tsleil-Waututh) Nations and elected/hereditary First Nation leaders across B.C. based on the principles of Rights, Recognition, and Respect</li>
    <li>Performs well under pressure, consistently meeting goals and deadlines, while maintaining a strong balance between family values and professional commitments; remains optimistic and resilient in the face of adversity</li>
    <li>Unequivocally believes that we must Paddle Together to achieve change (big or small)</li>
  </ul>
`;
export const resumeExecutiveExpText = `
    <h3 class="text-2xl font-light text-brandRed mb-6 text-center">Executive Experience</h3>
    <strong>Co-Founder & Partner, Aks Wind Energy Corp — Incorporated June 2025</strong>\n
    <br/>
    <br/>
    <strong>Founder & CEO Hli Haykwhl Ẃii Xsgaak Consutling Ltd Incorporated August 2023 </strong>
    <br/>
    <br/>
    <strong>Founder & CEO Remarkable First Nations Regenerative Industries Corp	Incorporated July 2023</strong>
    <br/>
    <br/>
    <strong>Member of the Legislative Assembly for Vancouver-Mount Pleasant</strong> Vancouver, B.C.<br/>
    Legislative Assembly of British Columbia	 February 2016 - April 2023<br/>
    <ul>
        <li>Elected as the first-ever First Nations woman to serve in the B.C. Legislature</li>
        <li>Represented 55,000 constituents across one of B.C.’s most diverse ridings, including the Downtown Eastside, Chinatown, Gastown, and Commercial Drive</li>
        <li>Advocated for constituents facing barriers accessing provincial services/funding (i.e., housing, small business, arts, health care, child welfare, education, etc.)</li>
        <li>Attended hundreds of community events to celebrate cultural diversity and raise awareness about Missing Murdered Indigenous Women and Girls</li>
        <li>Engaged multiple stakeholders through regular outreach to advocate on key issues and interests impacting residents, non-profit organizations, service providers, schools, industry, small businesses, and local government</li>
    </ul>
    <br/>

    <strong>Minister of Tourism, Arts, Culture and Sport	Victoria, B.C.</strong><br/>
    Government of British Columbia	November 2020 – September 2022<br/>
    <ul>
        <li>Served as a Member of the Executive Council and three Cabinet Committees (Planning and Priorities, Working Group on COVID, Economy); led a $338 million budget and 80 FTEs during COVID-19; delivered 90 percent of my Mandate Letter to advocate for B.C.’s hardest-hit sectors</li>
        <li>Overhauled the B.C. Arts Council; secured funding to host the 2024 Grey Cup, 2025 Invictus Games, and 2026 FIFA World Cup; established Vancouver’s first Chinese Canadian Museum in historic Chinatown; supported major redevelopments including the Jewish Community Centre of Greater Vancouver, Vancouver Art Gallery, and efforts to decolonize the Royal B.C. Museum</li>
    </ul>
    <br/>
    
    <strong>Minister of Advanced Education, Skills and Training	Victoria, B.C.</strong><br/>
    Government of British Columbia	 July 2017 – November 2020<br/>
    <ul>
        <li>Elected as the first-ever First Nations woman to serve in B.C.’s Cabinet</li>
        <li>Served as a Member of the Executive Council and three Cabinet Committees (Planning and Priorities, Social Initiatives, Reconciliation); led a $2.4 billion budget and 400 FTEs; delivered on my Mandate Letter while advancing key calls to action from the Truth and Reconciliation Commission, including funding the Indigenous language fluency degree (Article 16) and establishing the world’s first Indigenous law school at the University of Victoria (Article 50)</li>
        <li>Oversaw 25 public post-secondary institutions (PSIs); appointed 25% Indigenous people to the PSI boards; secured $450 million from the Treasury Board to build student housing; eliminated interest on student loans; created the Provincial Tuition Waiver program for former youth-in-care; initiated the B.C. Access Grant; led the Prevent Sexual Violence Campaign</li>
        <li>Invested in 2900 Technology seats; Open Text Books; Co-op and Work Integrated Learning opportunities; and Graduate Scholarships; advanced a unique partnership between the Urban Native Youth Association and the Nicola Valley Institute of B.C. to build the Indigenous Youth Centre</li>
        <li>Managed the Federal/Provincial/Territorial Labour Market Agreement; expanding skills and training opportunities for underrepresented groups; overhauled the Labour Market Outlook </li>
        <li>Reformed the Industry Training Authority (crown agency) by reducing barriers and increasing participation of youth, women, and Indigenous people in skilled trades/Red Seal certification</li>
    </ul>
    <br/>

    <strong>Associate Deputy Representative - Advocacy, Aboriginal, Community Relations & Youth Engagement	Vancouver, BC</strong><br/>
    BC Office of the Representative for Children & Youth	 January 2013 – March 2015<br/>
    <ul>
        <li>Served as a Member of the RCY’s senior Executive team, overseeing the organization's strategic direction, operations, reporting, and $7 million budget</li>
        <li>Cultivated trust and integrity as an independent Office of the Legislature by creating and maintaining collaborative relations with executive leaders of the Ministry for Children and Family Development, Community Living B.C., Indigenous communities, non-profit organizations, and the First Nations Directors Forum, representing 23 Delegated Aboriginal agencies</li>
        <li>Developed the RCY’s provincial youth engagement and community relations outreach strategies</li>
        <li>Delivered speeches and presentations on domestic violence, child rights, youth justice, and participation</li>
        <li>Managed 20 staff and the operations of the advocacy program; led complex advocacy cases involving vulnerable children, youth, and young adults</li>
    </ul>
    <br/>

    <strong>Child & Youth Advocate	Vancouver, BC</strong><br/>
    BC Office of the Representative for Children & Youth	 April 2007 – December 2012<br/>
    <ul>
        <li>Provided direct advocacy for the most vulnerable children, youth, and families across B.C.<li>
        <li>Facilitated child rights and advocacy workshops to raise awareness and empower individuals on RCY’s mandate<li>
        <li>Co-developed and designed Champions for Change, a booklet to empower self-advocacy skills</li>
    </ul>
    <br/>

    <strong>Additional Work, Volunteer, and Academic Experience</strong><br/>
    <ul>
        <li>Volunteer, Big Sisters (Nanaimo)</li>
        <li>Hostess/ Server, Red Robin Restaurants (North Vancouver)</li>
        <li>Indigenous Art Interpreter, Bill Reid Spirit of Haida Gwaii, YVR Airport Authority, (Vancouver)</li>
        <li>Practicum, Native Courtworker and Counselling Association of B.C. (Vancouver)</li>
        <li>National Aboriginal Project Coordinator, Sacred Lives Project, Save the Children Canada, (Nationwide, HQ Vancouver)</li>
        <li>Youth Advisory Member, Urban Multi-Purpose Aboriginal Youth Centres, Department of Canadian Heritage, (B.C. wide, Vancouver-based)</li>
        <li>Youth Worker, Covenant House (Vancouver)</li>
        <li>Practicum, Department of Justice (Vancouver)</li>
        <li>Practicum, RCMP National Indigenous Summer Student Program (Regina & Hazelton)</li>
    </ul>
`;
export const resumeCommunityInvolvementText = `
    <h3 class="text-2xl font-light text-brandRed mb-6 text-center">Community Involvement</h3>
    <ul>
        <li><strong>Partnership and Growth Strategy Advisor</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Hope and Health, British Columbia (2025 - Current)</p>
    <br/>

    <ul>
        <li><strong>Corporate Advisory Board Member	British Columbia</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;IGV Housing	2025 - Current</p>
    <br/>

    <ul>
        <li><strong>Chair / Advisor	Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Native Youth Centre $47 million Capital Campaign	2005 - 2009</p>
    <br/>

    <ul>
        <li><strong>Founder / President	Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Vancouver Aboriginal Community Policing Centre	2006 </p>
    <br/>

    <ul>
        <li><strong>President	Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Board of Directors, Urban Native Youth Association	2002 - 2006</p>
    <br/>
`;

export const resumeProfessionalDevText = `
    <h3 class="text-2xl font-light text-brandRed mb-6 text-center">Professional Development & Certifications</h3>
    <ul>
        <li><strong>Health & WellBeing - Dealing with Difficult Behaviour in the Workplace/ Workplace Conflict Management Strategies Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;BC Public Service Agency 2015</p>
    <br/>

    <ul>
        <li><strong>Advanced Executive Certificate in General Management Kingston, Ontario</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Queen’s Executive Program, Queens School of Business 2014</p>
    <br/>

    <ul>
        <li><strong>Indigenous Competency Training	Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Provincial Health Services Authority 2014</p>
    <br/>

    <ul>
        <li><strong>Financial Management 111 Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;BC Public Service Agency 2013</p>
    <br/>

    <ul>
        <li><strong>Building Strategic Alliances Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;BC Public Service Agency 2013</p>
    <br/>

    <ul>
        <li><strong>International Summer Course on the Rights of the Child	Moncton, NB</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;University of Moncton 2012</p>
    <br/>

    <ul>
        <li><strong>Child and Youth Human Rights Certificate Calgary, AB</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mount Royal University	2010</p>
    <br/>

    <ul>
        <li><strong>Conflict Resolution - Consensus Making and Group Facilitation	Vancouver, BC</strong></li>
    </ul>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Native Education College 1998</p>
    <br/>
`;
export const resumePostSecCredsText = `
    <h3 class="text-2xl font-light text-brandRed mb-6 text-center">Post-Secondary Credentials</h3>
    <p><strong>Simon Fraser University</strong> — Burnaby, B.C.<br/>Bachelor of Arts, Major in Political Science / Minor in Sociology (Fall 2002 – Spring 2005)</p>

    <br/>
    <p><strong>Native Education College / Douglas College Partnership</strong> — Vancouver, B.C.<br/>Criminology Diploma (1997 – 1999)</p>
`;
export const resumeAdditionalInfoText = `
    <h3 class="text-2xl font-light text-brandRed mb-6 text-center">Additional Information</h3>
    <strong>Media Relations</strong>
    <ul>
        <li><strong>Local</strong> - Roundhouse Radio, Vancouver Co-operative Radio, CKNW, Fanny Keefer Show, Vancouver Sun, The Province, Georgia Straight, Shared Vision, PowHERhouse, CBC TV/Radio, CTV, Global TV</li>
        <li><strong>National</strong> - Guest on Canada AM, Globe & Mail, APTN, Canadian Voices, CTV Question Period, Published article: My Life So Far - Canadian Women’s Studies, Chatelaine Magazine, Canadian Encyclopedia: https://www.thecanadianencyclopedia.ca/en/article/melanie-mark</li>
    </ul>
`;
export const resumeTravelExpText = `
    <h3 class="text-2xl font-light text-brandRed mb-6 text-center">Travel Experience</h3>
    <ul>
        <li><strong>B.C. & Canada</strong> - Extensive experience traveling across B.C. and Canada, professionally and personally</li>
        <li><strong>Santiago, Chile</strong> - Canadian delegate, United Nations Urban Indigenous Conference 2006</li>
        <li><strong>San Diego, California</strong> - Presented the Native Youth Centre Project to 1500 Aboriginal youth 2005</li>
        <li><strong>Philadelphia, USA</strong> - Delegate & presenter 2001</li>
        <li><strong>Yokohama, Japan</strong> - 2nd World Congress on Child & Youth Commercial Sexual Exploitation 2001</li>
        <li><strong>Personal travel</strong> - Cuba, Costa Rica, Croatia, Fiji, Mexico, New York City, Western Europe (England, Netherlands, Scotland, France, Portugal, Spain, Italy, Greece)</li>
    </ul>
`;
